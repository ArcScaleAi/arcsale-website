        ...
        <header className='w-full md:py-20 py-8'>
          <div className='w-11/12 mx-auto flex md:flex-row flex-col justify-between gap-10'>
            <div className='mt-20 basis-3/5'>
              <p className='md:text-[42px] text-3xl text-center md:text-start leading-8 font-bold md:leading-[52px]'>{<HighlightedTitle text={heroContent.title?.split('|')[0]}/>} {heroContent.title?.split('|')[1]}</p>
              <p className='py-10 text-xl leading-8 text-center md:text-start'>{heroContent.para?.split('|')[0]} {<HighlightedWord text={heroContent.para?.split('|')[1]} />} {heroContent.para?.split('|')[1]}</p>
              <DemoButton />
            </div>
